...
3. The Unit of Work pattern is used to group one or more operations (usually database CRUD operations) into a single transaction or “unit of work” 
4. so that all operations either pass or fail as one.
5. Things get complicated when a set of database commit operations on one entity depend on a commit operation on another entity.
6. instead we can further improve the design and efficiency of the data transactions across the application repository layers by clustering related repository components into a single unit, called as a **UnitOfWork**.
7. so we can reduce the number of times a database connection is made for transaction.
